#!/usr/bin/env python3
"""蓄势跨品牌运营分析看板 — 数据提取脚本（内联数据模式）
v2.1: 统一合约入口 — 五分类/SPUID/当前货号从 contract_lib 读取
"""
import pandas as pd
...
ord_df = ord_df[['货号', '数量', '出价金额（元）', '订单状态', '买家下单时间']].copy()
...
today = ALL_DATES[-1] if ALL_DATES else '2026-06-14'
def anomaly(window):
...
# ============================================================
print("📈 [3/6] 大盘...")
mkt = pd.read_excel(SRC, sheet_name='大盘手表指数', engine='calamine')
...
for _,r in mkt.iterrows():
    MARKET_MAP[r['date_str']] = round(r['idx'], 2)
...
comm = comm[['任务月份', '任务金额', '动态发布时间', '匹配货号', '曝光', '阅读数', '互动数', '商详访问', '任务状态']].copy()
...
push = push[['日期','商品ID','消耗(元)','直接支付单量(单)','直接支付金额(元)','引导支付单量(单)','引导支付金额(元)','货号']].copy()
...
HTML = f'''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>蓄势跨品牌运营分析看板</title><script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script><style>
...
<h1 style="color:#60a5fa;font-size:22px;margin-bottom:20px">📊 蓄势跨品牌运营分析看板</h1>
...
const ALL_BRANDS = {js_dumps(ALL_BRANDS)};
...
function updateM1(){{
  // DAILY_BRAND has brand field: {{date_str, brand, GMV, UV, orders}}
  // Aggregate across all brands for M1
  ...
  // Smart aggregation: sum GMV/UV/orders by date (multiple brands per date)
  if(!im&&!gf){{var byDate={{}};raw.forEach(function(r){{if(!byDate[r.date_str])byDate[r.date_str]={{GMV:0,UV:0,orders:0}};byDate[r.date_str].GMV+=r.GMV;byDate[r.date_str].UV+=r.UV;byDate[r.date_str].orders+=r.orders}});dates=Object.keys(byDate).sort();gmv=dates.map(function(d){{return byDate[d].GMV}});uv=dates.map(function(d){{return byDate[d].UV}});orders=dates.map(function(d){{return byDate[d].orders}})}}
...
kb = os.path.getsize(OUT_FILE) / 1024 / 1024
print(f"\n✅ {OUT_FILE} ({kb:.1f}MB inline HTML)")
print(f"   {ALL_DATES[0]} ~ {ALL_DATES[-1]}")
print(f"   Brands: {ALL_BRANDS}")
